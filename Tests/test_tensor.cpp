...
TEST(TensorTestCPU, im2col)
{
    ...
    EXPECT_THAT(col->values_vec(),
                Pointwise(FloatNear(1e-5),
                          std::vector<float>{1.0, 2.0, 4.0, 5.0,
                                            2.0, 3.0, 5.0, 6.0,
                                            4.0, 5.0, 7.0, 8.0,
                                            5.0, 6.0, 8.0, 9.0,
                                            }));
...
INSTANTIATE_TEST_SUITE_P(CPU, TensorTest, ::testing::Values(Device::CPU));
#ifdef CUDA_TEST
INSTANTIATE_TEST_SUITE_P(CUDA, TensorTest, ::testing::Values(Device::CUDA));
#endif
